...
from functions import get_CI, RD_attack, RCI_attack, graph_to_nn_set
...
def get_index_list(
    ...
    if attack in supported_attacks['initial']:
        ...
        logging.error(f'Attack {attack} not supported.')
...
def get_index_list_hybrid(
    ...
    if G.is_directed():
        logging.error('G must be undirected.')
        return 1

    if not G.is_simple():
        logging.error('G must be simple.')
        return 1
