...
# Example: echo $(seq 1 8) | xargs -n 1 -P 4 ./perc_parallel_fast.sh ER k5.00 16000 Ran True
...
OUTPUT="${NET_DIR}/${ATTACK}/comp_data_fast.txt"
if [ -f "${OUTPUT}" ]; then
    if [ ! -s "${OUTPUT}" ]; then
...
../perc_fast $NETWORK $ORDER $OUTPUT 
