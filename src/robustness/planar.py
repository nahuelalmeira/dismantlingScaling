...
def distance(
    s: Union[Tuple[float, float], int], 
    t: Union[Tuple[float, float], int], 
...
def create_points(N:int, d: int = 2, random_seed: Optional[int] = None):
    np.random.seed(random_seed)
    return np.random.random((N, d))

def replicate(points: Optional[Iterable[Tuple[float, float]]] = None):
    ...
    for i in [-1, 0, 1]:
        for j in [-1, 0, 1]:
            ...
            replicated_points = np.concatenate(
                (replicated_points, tile_points), axis=0
            )

    return replicated_points

def get_dt_simplices(
    ...
    relabeled_simplices = np.unique(
        np.sort(relabeled_simplices, axis=1), axis=0
    )

    return relabeled_simplices

def create_dt_edgelist(
    points: Iterable[Tuple[float, float]], 
    periodic: bool = False
):
...
def get_mst(edgelist, points, distances=None):
    ...
    edgelist = np.concatenate(
        (np.array(edgelist), np.flip(np.array(edgelist), axis=1))
    )

    nodes, indices, inverse = np.unique(
        edgelist, return_index=True, return_inverse=True
    )
    ...
    rows, cols = np.where(
        sp.sparse.csgraph.minimum_spanning_tree(matrix).toarray()
    )
...
def get_cell(point: Tuple[float, float], N: int) -> Tuple[int, int]:
    """
...
def build_cell_matrix(points: Iterable[Tuple[int, int]]):
...
def create_proximity_graph(
    ...
    distances: Optional[Iterable[Iterable[float]]] = None,
    random_seed: Optional[int] = None,
    meank: Optional[float] = None
) -> ig.Graph:
...
def get_r_from_meank(meank: float, N: int, correct: bool = True) -> float:
    """
